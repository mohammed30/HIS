import type { CreateInventoryCountDto, GetInventoryCountsInput, InventoryCountDto, UpdateInventoryCountItemDto } from './models';
...
export class InventoryCountService {
  ...
  getList = (input: GetInventoryCountsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<InventoryCountDto>>({
      method: 'GET',
      url: '/api/app/inventory-count',
      params: { warehouseId: input.warehouseId, status: input.status, fromDate: input.fromDate, toDate: input.toDate, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
  ...
  updateItem = (countId: string, input: UpdateInventoryCountItemDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      method: 'PUT',
      url: `/api/app/inventory-count/item/${countId}`,
      body: input,
    },
    { apiName: this.apiName,...config });
}