import type { AccountLookupDto, CreateUpdateJournalEntryDto, GetJournalEntriesInput, JournalEntryDto } from './dtos/models';
...
export class JournalEntryService {
  ...
  getList = (input: GetJournalEntriesInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<JournalEntryDto>>({
      method: 'GET',
      url: '/api/app/journal-entry',
      params: { dateFrom: input.dateFrom, dateTo: input.dateTo, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
