...
export class HRService {
  ...
  getPaySlipPdf = (payrollRunId: string, employeeId: string, config?: Partial<Rest.Config>) =>
  ...
  getPayrollRunEmployees = (payrollRunId: string, config?: Partial<Rest.Config>) =>
    this.restService.request<any, EmployeeLookupDto[]>({
      method: 'GET',
      url: `/api/app/h-r/payroll-run-employees/${payrollRunId}`,
    },
    { apiName: this.apiName,...config });
  ...
  importAttendance = (file: FormData, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      method: 'POST',
      url: '/api/app/h-r/import-attendance',
      body: file,
    },
    { apiName: this.apiName,...config });
