namespace HIS.Settings;

public static class HISSettings
{
    private const string Prefix = "HIS";

    public const string HospitalName = Prefix + ".HospitalName";
    public const string HospitalAddress = Prefix + ".HospitalAddress";
    public const string HospitalPhone = Prefix + ".HospitalPhone";
    public const string HospitalEmail = Prefix + ".HospitalEmail";
    public const string HospitalLogo = Prefix + ".HospitalLogo";
    public const string HospitalTaxNumber = Prefix + ".HospitalTaxNumber";

    public static class Pharmacy
    {
        public const string AllowNegativeStock = Prefix + ".Pharmacy.AllowNegativeStock";
    }

    public static class Inpatient
    {
        public const string AdmissionDepositAmount = Prefix + ".Inpatient.AdmissionDepositAmount";
        public const string RequireAdvancePayment = Prefix + ".Inpatient.RequireAdvancePayment";
    }

    public static class User
    {
        public const string Theme = Prefix + ".User.Theme";
    }
}
