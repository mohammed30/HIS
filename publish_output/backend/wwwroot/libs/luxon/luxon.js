    ...
    _proto4.weekdays = function weekdays$1(length, format) {
      ...
      return listStuff(this, length, weekdays, function () {
        var intl = format ? {
          ...
          formatStr = format ? "format" : "standalone";
        if (!_tHISDB.weekdaysCache[formatStr][length]) {
          _tHISDB.weekdaysCache[formatStr][length] = mapWeekdays(function (dt) {
            return _tHISDB.extract(dt, intl, "weekday");
          });
        }
        return _tHISDB.weekdaysCache[formatStr][length];
      });
    };
